...
export default function Students() {
  ...
  async function getSearchQuery() {
    ...
    try {
      const res = await Axios.get(`/users/search?query=${query}&role=Student`);
      console.log(res);
  ...
  const whichData = query.length > 0 ? resultsOfSearching : students;
  return (
    <>
      ...
      <div className="flex flex-col">
        <div className="flex flex-col md:flex-row justify-between gap-3 md:items-center mb-5">
          ...
          <Link
            to="add"
            className="w-[90%] self-end md:self-auto flex gap-2 items-center max-w-50 justify-center py-3 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 rounded-md text-white ring-2 font-poppins font-medium cursor-pointer"
          >
