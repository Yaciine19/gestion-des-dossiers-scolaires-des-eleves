...
export default function EditExam() {
  ...
  useEffect(() => {
    async function fetchExam() {
      try {
        ...
        const formatedDate = examData.date
          ? new Date(examData.date).toISOString().split("T")[0]
          : "";

        setForm({ ...examData, date: formatedDate });
        setIsDisabled(false);
      } catch (error) {
        console.log(error);
      }
  ...
  async function handleOnSubmit(e) {
    e.preventDefault();
    try {
      const res = await Axios.put(`/${EXAMS}/${id}`, form);
      setIsLoading(false);
      console.log(res);
      ...
      setTimeout(() => {
        navigate("/dashboard/exams", {
          state: {
            successMessage: {
              title: "Exam Udpated successfully!",
              message: "The Exam's data has been successfully updated.",
            },
          },
        });
      }, 100);
        ...
        <div className="rounded-lg shadow h-auto p-6 bg-white relative overflow-hidden">
          {isLoading ? (
            ...
            <form onSubmit={handleOnSubmit} className="w-full mt-8 space-y-6">
              <div className={`${!form.subject && "flex gap-3 items-center"}`}>
                <label
                  htmlFor="subject"
